...
type RelationshipQuery struct {
	...
	withFromContact *ContactQuery
	withToContact   *ContactQuery
...
// QueryFromContact chains the current query on the "from_contact" edge.
func (_q *RelationshipQuery) QueryFromContact() *ContactQuery {
...
// QueryToContact chains the current query on the "to_contact" edge.
func (_q *RelationshipQuery) QueryToContact() *ContactQuery {
	query := (&ContactClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *RelationshipQuery) Clone() *RelationshipQuery {
	...
	return &RelationshipQuery{
		...
		withFromContact: _q.withFromContact.Clone(),
		withToContact:   _q.withToContact.Clone(),
...
// WithFromContact tells the query-builder to eager-load the nodes that are connected to
// the "from_contact" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RelationshipQuery) WithFromContact(opts ...func(*ContactQuery)) *RelationshipQuery {
	query := (&ContactClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withFromContact = query
	return _q
}

// WithToContact tells the query-builder to eager-load the nodes that are connected to
// the "to_contact" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RelationshipQuery) WithToContact(opts ...func(*ContactQuery)) *RelationshipQuery {
	query := (&ContactClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withToContact = query
	return _q
}
...
//		GroupBy(relationship.FieldFromContactID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Relationship.Query().
//		Select(relationship.FieldFromContactID).
//		Scan(ctx, &v)
...
func (_q *RelationshipQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Relationship, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [2]bool{
			_q.withFromContact != nil,
			_q.withToContact != nil,
		}
	)
	...
	if query := _q.withFromContact; query != nil {
		if err := _q.loadFromContact(ctx, query, nodes, nil,
			func(n *Relationship, e *Contact) { n.Edges.FromContact = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withToContact; query != nil {
		if err := _q.loadToContact(ctx, query, nodes, nil,
			func(n *Relationship, e *Contact) { n.Edges.ToContact = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RelationshipQuery) loadFromContact(ctx context.Context, query *ContactQuery, nodes []*Relationship, init func(*Relationship), assign func(*Relationship, *Contact)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Relationship)
	for i := range nodes {
		fk := nodes[i].FromContactID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "from_contact_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *RelationshipQuery) loadToContact(ctx context.Context, query *ContactQuery, nodes []*Relationship, init func(*Relationship), assign func(*Relationship, *Contact)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Relationship)
	for i := range nodes {
		fk := nodes[i].ToContactID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "to_contact_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *RelationshipQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withFromContact != nil {
			_spec.Node.AddColumnOnce(relationship.FieldFromContactID)
		}
		if _q.withToContact != nil {
			_spec.Node.AddColumnOnce(relationship.FieldToContactID)
		}
	}
