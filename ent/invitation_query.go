...
func (_q *InvitationQuery) QueryChurch() *ChurchQuery {
...
// QueryAcceptedUser chains the current query on the "accepted_user" edge.
func (_q *InvitationQuery) QueryAcceptedUser() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *InvitationQuery) QueryCustomRole() *CustomRoleQuery {
...
func (_q *InvitationQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *InvitationQuery) FirstIDX(ctx context.Context) int {
...
func (_q *InvitationQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *InvitationQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *InvitationQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *InvitationQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *InvitationQuery) Clone() *InvitationQuery {
	...
	return &InvitationQuery{
		...
		withAcceptedUser: _q.withAcceptedUser.Clone(),
...
func (_q *InvitationQuery) WithChurch(opts ...func(*ChurchQuery)) *InvitationQuery {
	query := (&ChurchClient{config: _q.config}).Query()
...
// WithAcceptedUser tells the query-builder to eager-load the nodes that are connected to
// the "accepted_user" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *InvitationQuery) WithAcceptedUser(opts ...func(*UserQuery)) *InvitationQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAcceptedUser = query
	return _q
}
...
func (_q *InvitationQuery) WithCustomRole(opts ...func(*CustomRoleQuery)) *InvitationQuery {
	query := (&CustomRoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(invitation.FieldInviteeEmail).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Invitation.Query().
//		Select(invitation.FieldInviteeEmail).
//		Scan(ctx, &v)
...
func (_q *InvitationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Invitation, error) {
	var (
		...
		loadedTypes = [4]bool{
			_q.withChurch != nil,
			_q.withInviter != nil,
			_q.withAcceptedUser != nil,
			_q.withCustomRole != nil,
		}
	)
	if _q.withChurch != nil || _q.withInviter != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withChurch; query != nil {
		if err := _q.loadChurch(ctx, query, nodes, nil,
			func(n *Invitation, e *Church) { n.Edges.Church = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withAcceptedUser; query != nil {
		if err := _q.loadAcceptedUser(ctx, query, nodes, nil,
			func(n *Invitation, e *User) { n.Edges.AcceptedUser = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCustomRole; query != nil {
		if err := _q.loadCustomRole(ctx, query, nodes, nil,
			func(n *Invitation, e *CustomRole) { n.Edges.CustomRole = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *InvitationQuery) loadChurch(ctx context.Context, query *ChurchQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *Church)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Invitation)
	for i := range nodes {
		if nodes[i].church_invitations == nil {
			continue
		}
		fk := *nodes[i].church_invitations
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "church_invitations" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *InvitationQuery) loadInviter(ctx context.Context, query *UserQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *User)) error {
	...
	nodeids := make(map[int][]*Invitation)
	for i := range nodes {
		if nodes[i].user_sent_invitations == nil {
			continue
		}
		fk := *nodes[i].user_sent_invitations
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_sent_invitations" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *InvitationQuery) loadAcceptedUser(ctx context.Context, query *UserQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(invitation.AcceptedUserColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.invitation_accepted_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "invitation_accepted_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "invitation_accepted_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *InvitationQuery) loadCustomRole(ctx context.Context, query *CustomRoleQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *CustomRole)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Invitation)
	for i := range nodes {
...
func (_q *InvitationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withCustomRole != nil {
			_spec.Node.AddColumnOnce(invitation.FieldCustomRoleID)
		}
	}
