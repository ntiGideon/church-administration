...
func (_q *UserQuery) QueryChurch() *ChurchQuery {
...
func (_q *UserQuery) QueryContact() *ContactQuery {
...
// QueryFinanceRecords chains the current query on the "finance_records" edge.
func (_q *UserQuery) QueryFinanceRecords() *FinanceQuery {
...
// QueryAcceptedInvitation chains the current query on the "accepted_invitation" edge.
func (_q *UserQuery) QueryAcceptedInvitation() *InvitationQuery {
	query := (&InvitationClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *UserQuery) WithChurch(opts ...func(*ChurchQuery)) *UserQuery {
	query := (&ChurchClient{config: _q.config}).Query()
...
func (_q *UserQuery) WithContact(opts ...func(*ContactQuery)) *UserQuery {
	query := (&ContactClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithFinanceRecords tells the query-builder to eager-load the nodes that are connected to
// the "finance_records" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithFinanceRecords(opts ...func(*FinanceQuery)) *UserQuery {
	query := (&FinanceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withFinanceRecords = query
	return _q
}
...
// WithAcceptedInvitation tells the query-builder to eager-load the nodes that are connected to
// the "accepted_invitation" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithAcceptedInvitation(opts ...func(*InvitationQuery)) *UserQuery {
	query := (&InvitationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withChurch; query != nil {
		if err := _q.loadChurch(ctx, query, nodes, nil,
			func(n *User, e *Church) { n.Edges.Church = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withContact; query != nil {
		if err := _q.loadContact(ctx, query, nodes, nil,
			func(n *User, e *Contact) { n.Edges.Contact = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withFinanceRecords; query != nil {
		if err := _q.loadFinanceRecords(ctx, query, nodes,
			func(n *User) { n.Edges.FinanceRecords = []*Finance{} },
			func(n *User, e *Finance) { n.Edges.FinanceRecords = append(n.Edges.FinanceRecords, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadChurch(ctx context.Context, query *ChurchQuery, nodes []*User, init func(*User), assign func(*User, *Church)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*User)
	for i := range nodes {
		if nodes[i].church_users == nil {
			continue
		}
		fk := *nodes[i].church_users
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "church_users" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *UserQuery) loadContact(ctx context.Context, query *ContactQuery, nodes []*User, init func(*User), assign func(*User, *Contact)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*User)
	for i := range nodes {
		if nodes[i].contact_user == nil {
			continue
		}
		fk := *nodes[i].contact_user
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "contact_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *UserQuery) loadFinanceRecords(ctx context.Context, query *FinanceQuery, nodes []*User, init func(*User), assign func(*User, *Finance)) error {
	...
	query.Where(predicate.Finance(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.FinanceRecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_finance_records
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_finance_records" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_finance_records" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *UserQuery) loadAnnouncements(ctx context.Context, query *AnnouncementQuery, nodes []*User, init func(*User), assign func(*User, *Announcement)) error {
	...
	for _, n := range neighbors {
		fk := n.user_announcements
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_announcements" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_announcements" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserQuery) loadAcceptedInvitation(ctx context.Context, query *InvitationQuery, nodes []*User, init func(*User), assign func(*User, *Invitation)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*User)
	for i := range nodes {
		if nodes[i].invitation_accepted_user == nil {
			continue
		}
		fk := *nodes[i].invitation_accepted_user
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "invitation_accepted_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
