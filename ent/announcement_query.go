...
func (_q *AnnouncementQuery) QueryChurch() *ChurchQuery {
...
func (_q *AnnouncementQuery) QueryAuthor() *UserQuery {
...
func (_q *AnnouncementQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *AnnouncementQuery) FirstIDX(ctx context.Context) int {
...
func (_q *AnnouncementQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *AnnouncementQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *AnnouncementQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *AnnouncementQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *AnnouncementQuery) WithChurch(opts ...func(*ChurchQuery)) *AnnouncementQuery {
	query := (&ChurchClient{config: _q.config}).Query()
...
func (_q *AnnouncementQuery) WithAuthor(opts ...func(*UserQuery)) *AnnouncementQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AnnouncementQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Announcement, error) {
	...
	if _q.withChurch != nil || _q.withAuthor != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withChurch; query != nil {
		if err := _q.loadChurch(ctx, query, nodes, nil,
			func(n *Announcement, e *Church) { n.Edges.Church = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAuthor; query != nil {
		if err := _q.loadAuthor(ctx, query, nodes, nil,
			func(n *Announcement, e *User) { n.Edges.Author = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AnnouncementQuery) loadChurch(ctx context.Context, query *ChurchQuery, nodes []*Announcement, init func(*Announcement), assign func(*Announcement, *Church)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Announcement)
	for i := range nodes {
		if nodes[i].church_announcements == nil {
			continue
		}
		fk := *nodes[i].church_announcements
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "church_announcements" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *AnnouncementQuery) loadAuthor(ctx context.Context, query *UserQuery, nodes []*Announcement, init func(*Announcement), assign func(*Announcement, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Announcement)
	for i := range nodes {
		if nodes[i].user_announcements == nil {
			continue
		}
		fk := *nodes[i].user_announcements
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_announcements" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *AnnouncementQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(announcement.Table, announcement.Columns, sqlgraph.NewFieldSpec(announcement.FieldID, field.TypeInt))
	_spec.From = _q.sql
