...
func (_q *EventQuery) QueryChurch() *ChurchQuery {
...
func (_q *EventQuery) WithChurch(opts ...func(*ChurchQuery)) *EventQuery {
	query := (&ChurchClient{config: _q.config}).Query()
...
func (_q *EventQuery) loadChurch(ctx context.Context, query *ChurchQuery, nodes []*Event, init func(*Event), assign func(*Event, *Church)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
		if nodes[i].church_events == nil {
			continue
		}
		fk := *nodes[i].church_events
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "church_events" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
