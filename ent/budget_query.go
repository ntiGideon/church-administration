...
func (_q *BudgetQuery) QueryChurch() *ChurchQuery {
...
func (_q *BudgetQuery) QueryLines() *BudgetLineQuery {
	query := (&BudgetLineClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *BudgetQuery) WithChurch(opts ...func(*ChurchQuery)) *BudgetQuery {
	query := (&ChurchClient{config: _q.config}).Query()
...
func (_q *BudgetQuery) WithLines(opts ...func(*BudgetLineQuery)) *BudgetQuery {
	query := (&BudgetLineClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *BudgetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Budget, error) {
	...
	if query := _q.withChurch; query != nil {
		if err := _q.loadChurch(ctx, query, nodes, nil,
			func(n *Budget, e *Church) { n.Edges.Church = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withLines; query != nil {
		if err := _q.loadLines(ctx, query, nodes,
			func(n *Budget) { n.Edges.Lines = []*BudgetLine{} },
			func(n *Budget, e *BudgetLine) { n.Edges.Lines = append(n.Edges.Lines, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *BudgetQuery) loadChurch(ctx context.Context, query *ChurchQuery, nodes []*Budget, init func(*Budget), assign func(*Budget, *Church)) error {
...
func (_q *BudgetQuery) loadLines(ctx context.Context, query *BudgetLineQuery, nodes []*Budget, init func(*Budget), assign func(*Budget, *BudgetLine)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(budgetline.FieldBudgetID)
	}
	query.Where(predicate.BudgetLine(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(budget.LinesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *BudgetQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withChurch != nil {
			_spec.Node.AddColumnOnce(budget.FieldChurchID)
		}
	}
